//  LAST EDIT: Fri Jan 12 11:02:00 2002 by Vivian De Smedt
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Copyright (C) 2001, Vivian De Smedt
...
void OsiGlpkSolverInterfaceUnitTest( const std::string & mpsDir, const std::string & netlibDir )
{
...

  {
    ...
    // Test clone
    {
      OsiGlpkSolverInterface cplexSi(m);
      OsiSolverInterface * siPtr = &cplexSi;
      OsiSolverInterface * siClone = siPtr->clone();
      OsiGlpkSolverInterface * cplexClone = dynamic_cast<OsiGlpkSolverInterface*>(siClone);
    ...
    // Test apply cuts method
    {
	  ...
	  // Temporarily added to avoid 0 element (other glp complains)
	  el[0] = 0.5;
...
#if 0
    {
      ...
      assert( eq(cs[7],8.0) );
      {
        OsiGlpkSolverInterface solnSi(cplexSi);
